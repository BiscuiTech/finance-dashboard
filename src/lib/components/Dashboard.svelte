	...
	<div class="my-2  sm:-mx-6 lg:-mx-8">
		<div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
			<div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                ...
                {item.averageEntryPrice >= item.currentPrice ? 'text-red-500' : 'text-green-500'}"
								>
									{((item.currentPrice / item.averageEntryPrice - 1) * 100).toFixed(2)}%
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-700">
									${item.currentMarketValue}
									<span class="inline-block bg-gray-200 text-gray-700 rounded p-1 ml-1"
										>({((item.currentMarketValue / totalMarketValue) * 100).toFixed(0)}%)</span
									>
								</td>
